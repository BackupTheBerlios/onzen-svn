...
*
* $Revision: 1472 $
* $Date: 2013-09-30 22:19:44 +0200 (Mo, 30 Sep 2013) $
* $Author: trupp $
* Contents: command copy files/directories
* Systems: all
*
\***********************************************************************/
...
class CommandCopy
{
   ...
   */
  CommandCopy(final Shell shell, final RepositoryTab repositoryTab, final HashSet<FileData> fileDataSet)
  {
    ...
    // copy file dialog
    dialog = Dialogs.openModal(shell,"Copy files",new double[]{1.0,0.0},1.0);
    ...
    Widgets.layout(composite,0,0,TableLayoutData.NSWE,0,0,4);
    {
      ...
      {
        label = Widgets.newLabel(subComposite,"Destination:");
        Widgets.layout(label,0,0,TableLayoutData.W);

        widgetDestination = Widgets.newText(subComposite);
        Widgets.layout(widgetDestination,0,1,TableLayoutData.WE);
        ...
        button.addSelectionListener(new SelectionListener()
        {
          ...
          {
            File file = new File(repositoryTab.repository.rootPath,widgetDestination.getText());
            ...
            if (newDirectory != null)
            {
              ...
              widgetDestination.setText(repositoryTab.repository.getFileName(newFile));
            }
          }
        });
      }
      ...
      widgetMessage.setToolTipText("Commit message. Use:\n"+
                                   ...
                                   Widgets.acceleratorToText(Settings.keyCommitMessageDone)+" to copy files."
                                  );
    ...
    Widgets.layout(composite,1,0,TableLayoutData.WE,0,0,4);
    {
      ...
      Widgets.layout(widgetCopy,0,0,TableLayoutData.W,0,0,0,0,80,SWT.DEFAULT);
    ...
    // listeners
    widgetDestination.addModifyListener(new ModifyListener()
    {
      public void modifyText(ModifyEvent modifyEvent)
      {
        Text widget = (Text)modifyEvent.widget;

        widgetCopy.setEnabled(!widget.getText().isEmpty());
      }
    });
    Widgets.setNextFocus(widgetDestination,widgetMessage);
    ...
    // show dialog
    Dialogs.show(dialog,Settings.geometryCopy,Settings.setWindowLocation);
  ...
  public void run()
  {
    Widgets.setFocus(widgetDestination);
  ...
  private void copy()
  {
    repositoryTab.setStatusText("Copy files to '%s'...",data.destination);
    ...
    try
    {
      ...
      // copy files
      repositoryTab.repository.copy(fileDataSet,data.destination,commitMessage);

      // update file states
      FileData fileData = new FileData(data.destination);
